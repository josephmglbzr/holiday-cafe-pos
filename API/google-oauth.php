...
$google_oauth_client_id = '4776894337-stl4v89lhaiqcji961oglkq8qothhtd4.apps.googleusercontent.com';
$google_oauth_client_secret = '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';
$google_oauth_redirect_uri = 'http://localhost/holiday-cafe-pos-system/API/google-oauth.php';
...
if (isset($_GET['code']) && !empty($_GET['code'])) {
    ...
    if (isset($response['access_token']) && !empty($response['access_token'])) {
        ...
        if (isset($profile['email'])) {
            ...
            $stmt = $pdo->prepare('SELECT * FROM google_accounts WHERE email = ?');
            $stmt->execute([ $profile['email'] ]);
            $account = $stmt->fetch(PDO::FETCH_ASSOC);
            // If the account does not exist in the database, insert the account into the database
            if (!$account) {
                $stmt = $pdo->prepare('INSERT INTO google_accounts (email, name, picture, registered, method) VALUES (?, ?, ?, ?, ?)');
            ...
            header('Location:'.SITENEW.'customer-profile.php');
            exit;
        } else {
